...
<body x-data="{ sidebarOpen: false, notifOpen: false }" class="bg-gradient-to-r from-pink-100 via-pink-200 to-rose-100"
 min-h-screen text-gray-800">
    ...
    <div x-show="sidebarOpen" class="fixed inset-0 z-50 flex" x-cloak>
        ...
        <aside class="relative w-64 bg-white shadow-lg z-50 p-6 space-y-4">
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-xl font-bold text-pink-700">Menu</h2>
                <button @click="sidebarOpen = false" class="text-gray-600 hover:text-red-600">✕</button>
            </div>
            <nav class="space-y-2">
            <a href="{{ route('bookings.create') }}"
            class="block px-4 py-2 rounded hover:bg-pink-100 transition {{ request()->routeIs('bookings.create') ? 'bg-pink-100 font-semibold text-pink-700' : 'text-gray-700' }}">
                ➕ Create Booking
            </a>
            
            <a href="{{ route('bookings.index') }}"
            class="block px-4 py-2 rounded hover:bg-pink-100 transition {{ request()->routeIs('bookings.index') ? 'bg-pink-100 font-semibold text-pink-700' : 'text-gray-700' }}">
                📋 View Bookings
            </a>

            @can('viewAny', App\Models\Booking::class)
                <a href="{{ route('admin.bookings') }}"
                class="block px-4 py-2 rounded hover:bg-pink-100 transition {{ request()->routeIs('admin.bookings') ? 'bg-pink-100 font-semibold text-pink-700' : 'text-gray-700' }}">
                    🛠 All Bookings (Admin)
                </a>
            @endcan

            <a href="{{ route('profile.edit') }}"
            class="block px-4 py-2 rounded hover:bg-pink100 transition {{ request()->routeIs('profile.edit') ? 'bg-pink-100 font-semibold text-pink-700' : 'text-gray-700' }}">
                👤 Profile
            </a>
        </nav>
    ...
    <nav class="bg-white shadow-md h-16 flex items-center justify-between px-4 sm:px-6 lg:px-8 sticky top-0 z-40">
        ...
        <div class="flex items-center space-x-4">
            <button @click="sidebarOpen = true" class="text-pink-700 focus:outline-none">
                <svg class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                          d="M4 6h16M4 12h16M4 18h16" />
                </svg>
            </button>
            <a href="{{ route('dashboard') }}" class="text-lg font-bold text-pink-700">Dashboard</a>
        </div>
